...
- Fixed XCP-ng XenAPI integer encoding, trusted endpoint configuration, template validation, collision-resistant ISO runs, required networking for fresh ISO VMs, bounded guest-network discovery, failure-recoverable VM ownership, copied-disk and local-key cleanup, generated Windows answer media, pre-boot answer attachment, and bounded ISO E2E cleanup.

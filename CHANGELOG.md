...
- Local containers: recognize Podman's exact quoted-ID absence diagnostic so a retried stop can finish cleanup after the container disappears, while retaining recovery state for ambiguous or inaccessible runtimes. [PR 1922](https://github.com/openclaw/crabbox/pull/1922).
...
- Apple Machine: honor cancellation through ownership waits and native controls, include claim-lock waiting in cleanup budgets, and preserve the original failure when acquisition rollback also fails. [PR 1917](https://github.com/openclaw/crabbox/pull/1917).
...
- Crownest: accept but omit framework-owned run bookkeeping without enabling command env forwarding, preserve primary outcomes and typed cleanup errors in final timing, and retain recovery claims when Workspace Run cancellation fails. [PR 1927](https://github.com/openclaw/crabbox/pull/1927).
- Cloud Run Sandbox: preserve direct command exits and primary typed failures through activity cleanup, teardown and reporting, with terminal timing and recovery-session reporting based on the final outcome. [PR 1928](https://github.com/openclaw/crabbox/pull/1928).
...
- Reject conflicting known provider identities before rebinding resolved SSH access, and retain alias keys unless an attested unchanged alias claim is removed. [Issue 1900](https://github.com/openclaw/crabbox/issues/1900). Thanks @coygeek.
...
- Orgo: share failure finalization so cleanup and timing errors preserve primary command/API exit codes and no longer report success after failed cleanup; retain workspace ownership and no-copy behavior. [PR 1926](https://github.com/openclaw/crabbox/pull/1926).

...
- Serialize fixed Proxmox VMID reservation through durable claim publication so concurrent local acquisitions cannot submit clones for the same ID.
...
- Commit legacy coordinator lease admission and publication atomically, resolving ambiguous commits through exact bound-record rereads without duplicate provisioning. [Issue 1561](https://github.com/openclaw/crabbox/issues/1561).
- Keep coordinator provisioning-failure interpretation provider-owned while core controls recovery delays and retains unresolved canceled allocations without a cloud ID. [PR 2478](https://github.com/openclaw/crabbox/pull/2478). Thanks @steipete.

...
- Added opt-in Linux Git overlay sync that resets runners to an exact remote-backed tree with hooks disabled across clone, fetch, and checkout and with checkout transforms isolated, preserves vetted ignored dependency caches while scrubbing stale state, uploads only the canonical dirty file set, falls back for checkout transforms or moved refs, reports transfer and fallback telemetry, and keeps transport failures fatal; ready-pool scrub now fetches only its requested branch without tags.

...
- AWS Lambda MicroVM: share failure finalization so termination and timing errors preserve primary outcomes, setup failures retain recovery metadata, and transport failures are not reported as command exits; keep successful claim-refresh and operation-lock semantics unchanged.

...
- Islo: finalize bound-run outcomes after guarded cleanup, retain setup failures with `--keep-on-failure` and reused sessions after admitted Tailscale preparation fails, and preserve primary codes/causes through cleanup or timing failures while distinguishing provider/artifact errors from command exits.

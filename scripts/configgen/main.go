...
func generate(s schema, source string) ([]byte, error) {
	...
	hasFlags, needsOS, needsStrings, needsTime := false, false, false, false
	...
	if hasFlags || needsOS || needsStrings || needsTime {
		p("import (")
		if hasFlags {
			p("\"flag\";")
		}
	...
	for _, f := range s.fields {
		...
		switch f.kind {
		case "time.Duration":
			p("if value := os.Getenv(%q); value != \"\" { applyLeaseDuration(&cfg.%s, value) }\n", f.env, f.name)
		case "string":

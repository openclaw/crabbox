...
for name in AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_SESSION_TOKEN CLOUDFLARE_API_TOKEN \
  QUALIFICATION_CONTROLLER_TOKEN QUALIFICATION_ADMIN_TOKEN QUALIFICATION_SHARED_TOKEN \
  QUALIFICATION_PREFLIGHT_TOKEN ACTIONS_ID_TOKEN_REQUEST_TOKEN ACTIONS_ID_TOKEN_REQUEST_URL; do
  if [[ -n "${!name+x}" ]]; then

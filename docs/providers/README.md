...
| [agx](agx.md) | built-in; `ssh-lease` · direct-cloud | Provider-specific SSH; `crabbox-sync` · direct only; features: `ssh`, `crabbox-sync` | `linux`; AGX microVM (Firecracker/Drafter) | `provider-managed`; GPU: no | AGX SSH gateway (provision on connect); local claim release; AGX reclaims idle sandboxes | Fast Linux microVM over the AGX SSH workspace gateway with your own SSH key | Early access (ships Summer 2026); AGX publishes no control-plane API, so leases are SSH-only with a BYO key and there is no remote list/cleanup |

...
| [railway](railway.md) (`rail`, `railwayapp`) | specialized; `service-control` · service-control | SSH not applicable; `none` · direct only; features: `url-bridge` | `linux`; Railway service | `cloud`; GPU: unknown | Railway; none; Railway-owned | Inspecting an existing Railway service | Cannot execute commands or stop unclaimed services |

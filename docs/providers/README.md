...
| --- | --- | --- | --- | --- | --- | --- | --- |
| [agent-sandbox](agent-sandbox.md) | built-in; `delegated-run` · delegated-sandbox | No SSH; `archive-sync` · direct only; features: `archive-sync`, `cleanup`, `run-session`, `fixed-current-repo-stop` | `linux`; Kubernetes Agent Sandbox warm pool | `self-hosted`; GPU: unknown | Agent Sandbox SandboxClaim; owned SandboxClaim delete | Kubernetes-hosted delegated Linux execution | Requires kubectl, Agent Sandbox v0.5.0rc1 v1beta1 CRDs, a warm pool, explicit context, and RBAC |
...
| [ascii-box](ascii-box.md) (`boat`, `ascii`, `asciibox`) | built-in; `ssh-lease` · direct-cloud | Crabbox-managed SSH; `crabbox-sync` · direct only; features: `ssh`, `crabbox-sync` | `linux`; Boat (formerly ASCII Box) managed Linux box | `provider-managed`; GPU: unknown | provider CLI; keyed API fixed lease IDs; provider delete | Managed Linux box over SSH, selectable as boat | Requires Boat/Box CLI; fixed IDs allow one keyed recovery within 24h; no account attestation |
| [aws](aws.md) | built-in; `ssh-lease` · brokerable-cloud | Crabbox-managed SSH; `crabbox-sync` · coordinator optional; features: `ssh`, `crabbox-sync`, `cleanup`, `desktop`, `browser`, `code`, `run-session`, `tailscale` | `linux`, `windows/normal`, `windows/wsl2`, `macos`; EC2 VM or dedicated Mac host | `cloud`; GPU: optional | Crabbox or coordinator; instance termination | Broad Linux, Windows, WSL2, and macOS cloud coverage | Largest configuration, quota, and cost surface |
...
| [parallels](parallels.md) | built-in; `ssh-lease` · local-vm | Crabbox-managed SSH; `crabbox-sync` · direct only; features: `ssh`, `crabbox-sync`, `cleanup`, `desktop`, `browser`, `code`, `workspace-checkpoint`, `workspace-fork`, `workspace-restore`, `provider-snapshot` | `linux`, `macos`, `windows/normal`, `windows/wsl2`; Parallels linked-clone VM | `local`; GPU: no | Crabbox; clone delete | Local macOS, Linux, or Windows VM with snapshots | Requires prepared source VMs and SSH; fixed lease IDs bind attested host and clone identity and remain single-use |
...
| [proxmox](proxmox.md) | built-in; `ssh-lease` · self-hosted-virtualization | Crabbox-managed SSH; `crabbox-sync` · direct only; features: `ssh`, `crabbox-sync`, `cleanup` | `linux`; Proxmox VE QEMU clone | `self-hosted`; GPU: optional | Crabbox; VM delete | Self-hosted Linux VM fleet | Needs a prepared template, guest agent, and network; fixed lease IDs bind VMID and generation and retain uncertain attempts |

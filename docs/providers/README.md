...
| [boxd](boxd.md) | built-in; `ssh-lease` · direct-cloud | Crabbox-managed SSH; `crabbox-sync` · direct only; features: `ssh`, `crabbox-sync`, `cleanup` | `linux`; boxd KVM microVM (Ubuntu 24.04) | `provider-managed`; GPU: no | boxd CLI; machine destroy (claim-gated) | Fast-boot isolated Linux microVMs over SSH with Docker preinstalled | Requires the boxd CLI and its auth (boxd auth login or BOXD_TOKEN); machine sizing follows the boxd account quota; machines are always created isolated |

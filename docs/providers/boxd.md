...
Host trust is the vendor-managed pin, and it is required: the CLI pre-trusts
the proxy host key in `~/.ssh/known_hosts` for every machine `[host]:port`,
the target verifies against that file, and the pin is enforced with
`StrictHostKeyChecking=yes` — a mismatched edge key is rejected, never trusted
on first use. When the pin is not yet present the provider re-syncs and
retries within the readiness window, then fails closed rather than trusting a
first connection.

...
- `cloudflare-dynamic-workers` — accepts a local Dynamic Workers claim, stable
  run ID, explicit run ID, or slug, then asks the loader for run metadata. The
  status target is `worker-runtime`, not a Linux host.

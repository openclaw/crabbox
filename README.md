...
Behind that single command: a Go CLI on your laptop, a Cloudflare Worker broker that owns provider credentials and lease state, and a vanilla Ubuntu runner on Hetzner Cloud or AWS EC2 Spot. Crabbox can also wrap Blacksmith Testboxes when you choose `provider: blacksmith-testbox`, target [islo.dev](https://islo.dev) sandboxes via the islo Go SDK with `provider: islo`, or use `provider: ssh` for existing macOS and Windows targets.
...
- **Islo SDK provider.** Set `provider: islo` and `ISLO_API_KEY` to drive [islo.dev](https://islo.dev) sandboxes through the [islo Go SDK](https://github.com/islo-labs/go-sdk); no `islo` CLI required.

...
// Hermetic lifecycle suite for the boxd provider: acquire, resolve, release,
// list, and cleanup behavior against a scripted CLI runner — no live
// credentials. Named per the *lifecycle*_test.go convention so
// scripts/test-provider-lifecycles.sh includes this package. Shared fakes and
// helpers live in backend_test.go (same package).

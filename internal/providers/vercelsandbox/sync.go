...
func (b *backend) syncWorkspace(ctx context.Context, api vercelSandboxClient, sandboxID string, req RunRequest, workdir string) ([]timingPhase, time.Duration, error) {
	...
	if err := checkVercelSandboxSyncPreflight(manifest, b.cfg, req.ForceSyncLarge, b.rt.Stderr); err != nil {
		return nil, 0, err
	}
	preflightDuration := b.now().Sub(preflightStart)

	archiveStart := b.now()
	archive, err := createVercelSandboxSyncArchive(syncCtx, req.Repo, manifest)
	if err != nil {
		return nil, 0, err
	}
	defer func() {
	...
	remoteArchive := path.Join("/tmp", "crabbox-vercel-sync-"+randomSuffix()+".tgz")
	extractDir := workdir
	stagingDir := ""
	...
	phases = append(phases, timingPhase{Name: "vercel_sandbox_sync", Ms: total.Milliseconds()})
	return phases, total, nil
}

func checkVercelSandboxSyncPreflight(manifest SyncManifest, cfg Config, force bool, stderr io.Writer) error {
...
func (b *backend) replaceWorkspace(ctx context.Context, api vercelSandboxClient, sandboxID, stagingDir, workdir string) error {
	...
	if err := b.execShell(ctx, api, sandboxID, "rm -rf "+shellQuote(backupDir)); err != nil {
		fmt.Fprintf(b.rt.Stderr, "warning: vercel-sandbox previous workspace cleanup failed path=%s: %v\n", backupDir, err)
	}
	return nil
}
...
func createVercelSandboxSyncArchive(ctx context.Context, repo Repo, manifest SyncManifest) (*os.File, error) {
	return createPortableSyncArchive(ctx, repo, manifest, "crabbox-vercel-sandbox-sync-*.tgz")
}

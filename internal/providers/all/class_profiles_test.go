...
func TestProductionProviderClassCatalogCompleteness(t *testing.T) {
	...
	if counts[core.ProviderClassDispositionMapped] != 14 || counts[core.ProviderClassDispositionUnmapped] != 67 || len(counts) != 2 {
		t.Fatalf("class disposition counts=%v want mapped=14 unmapped=67", counts)
	}
}

...
func TestIncusStopCLIForceDoesNotForgetTerminalReceipt(t *testing.T) {
	fake, lease, app := incusStopFixture(t)
	args := []string{"stop", "--provider", "incus", "--id", lease.LeaseID}
	...
	// Incus fixed receipts stay with their release owner, outside generic absence recovery.
	err = app.Run(t.Context(), append(args, "--force"))
	if err == nil || !strings.Contains(err.Error(), "does not support verified forced recovery") {
		t.Fatalf("unexpected forced recovery result: %v", err)
	}
	after, err := core.ReadLeaseClaim(lease.LeaseID)
	if err != nil || !reflect.DeepEqual(terminal, after) || len(fake.deleted) != 1 {
		t.Fatalf("forced recovery changed terminal custody: deletes=%v err=%v", fake.deleted, err)
	}
	if err := app.Run(t.Context(), args); err != nil {
		t.Fatalf("ordinary stop could not replay the retained receipt: %v", err)
	}
}

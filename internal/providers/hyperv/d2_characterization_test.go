...
func TestD2ObservationPrecedence(t *testing.T) {
	...
	for _, running := range []bool{true, false} {
		for _, stored := range []string{"", "ready", "error", "running", " "} {
			...
			view := b.serverFromInstance(hypervVM{Name: "crabbox-d2", State: native}, claim, cfg)
			wantStatus := "stopped"
			if running {
				wantStatus = "running"
				if stored == "ready" {
...
func TestD2ListDisplayFilterAndOrder(t *testing.T) {
	...
	payload := `[{"Name":"foreign","State":2},{"Name":"crabbox-second","State":3},{"Name":"crabbox-first","State":2}]`
	oldOS := hypervHostOS
	hypervHostOS = "windows"
	t.Cleanup(func() { hypervHostOS = oldOS })
	runner := &recordingRunner{respond: func(core.LocalCommandRequest) (core.LocalCommandResult, error, bool) {
		return core.LocalCommandResult{Stdout: payload}, nil, true
	}}

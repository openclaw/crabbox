...
func (r *parallelsFixedFleetRunner) Run(_ context.Context, req core.LocalCommandRequest) (core.LocalCommandResult, error) {
	...
	target := req.Args[len(req.Args)-2]
...
// R2c — the same machine reached through a different address or account still
// owns its lease.
//
...
func TestParallelsFixedFollowsHostToANewAddress(t *testing.T) {
	...
	// The same machine now answers at a new address, under a new account and a
	// new fleet entry name.
	runner.mu.Lock()
	moved := runner.machines[fleetMachineA]
	runner.machines[fleetMachineB] = moved
	runner.mu.Unlock()
	backend.Cfg.Parallels.Hosts = []core.ParallelsHostConfig{{Name: "fleet-renamed", Host: fleetMachineB, User: "operator"}}

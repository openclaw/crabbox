...
func (b *codeSandboxBackend) syncWorkspace(ctx context.Context, api codeSandboxAPI, sandboxID string, req RunRequest, workdir string) ([]timingPhase, time.Duration, error) {
	...
	if err := checkCodeSandboxSyncPreflight(manifest, b.cfg, req.ForceSyncLarge, b.rt.Stderr); err != nil {
		return nil, 0, err
	}
	preflightDuration := b.now().Sub(preflightStart)

	archiveStart := b.now()
	archive, err := createCodeSandboxSyncArchive(syncCtx, req.Repo, manifest)
	if err != nil {
		return nil, 0, err
	}
	defer func() {
	...
	uploadArchive := ".crabbox-codesandbox-sync-" + randomSuffix() + ".tgz"
	remoteArchive := path.Join(defaultWorkdir, uploadArchive)
	extractDir := workdir
	stagingDir := ""
	...
	phases = append(phases, timingPhase{Name: "codesandbox_sync", Ms: total.Milliseconds()})
	return phases, total, nil
}

func checkCodeSandboxSyncPreflight(manifest SyncManifest, cfg Config, force bool, stderr io.Writer) error {
...
func (b *codeSandboxBackend) replaceWorkspace(ctx context.Context, api codeSandboxAPI, sandboxID, stagingDir, workdir string) error {
	...
	if err := b.execShell(ctx, api, sandboxID, "rm -rf "+shellQuote(backupDir)); err != nil {
		fmt.Fprintf(b.rt.Stderr, "warning: codesandbox previous workspace cleanup failed path=%s: %v\n", backupDir, err)
	}
	return nil
}
...
func createCodeSandboxSyncArchive(ctx context.Context, repo Repo, manifest SyncManifest) (*os.File, error) {
	return createPortableSyncArchive(ctx, repo, manifest, "crabbox-codesandbox-sync-*.tgz")
}

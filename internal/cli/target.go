...
func validateProviderTarget(cfg Config) error {
	...
	if machineTarget && effectiveArchitectureForConfig(cfg) == ArchitectureARM64 {
		if provider.Name() != "azure" && provider.Name() != "aws" && provider.Name() != "tart" && provider.Name() != "apple-vm" && provider.Name() != "lume" && provider.Name() != "aws-lambda-microvm" {
			return exit(2, "architecture=arm64 currently supports provider=azure, provider=aws, provider=tart, provider=apple-vm, provider=lume, or provider=aws-lambda-microvm")
		}
		if cfg.TargetOS != targetLinux &&
			!(provider.Name() == "azure" && cfg.TargetOS == targetWindows) &&
			!((provider.Name() == "tart" || provider.Name() == "lume") && cfg.TargetOS == targetMacOS) {
			return exit(2, "architecture=arm64 currently supports target=linux, provider=azure target=windows, or provider=tart/provider=lume target=macos only")
		}

...
func (r *boundedReadSeeker) Read(data []byte) (int, error) {
	if r.maxAllowed > 0 && len(data) > r.maxAllowed {
		return 0, fmt.Errorf("unbounded read buffer: %d", len(data))
	}
...
func TestWorkspaceOwnerWSL2SSHHelpersStreamExpandedCommands(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			script := owner.wrapPOSIXCommand(remote, test.preserveInput, workspaceOwnerMarkerFromTestFrame(t, input))
...
func TestWorkspaceOwnerWSL2SSHInputReplaysIdenticalFallbackFrame(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, firstInput := readWorkspaceOwnerSSHCall(t, dir, 1)
			script := owner.wrapPOSIXCommand(remote, true, workspaceOwnerMarkerFromTestFrame(t, firstInput))
...
func workspaceOwnerMarkerFromTestFrame(t *testing.T, frame string) string {
	t.Helper()
	_, rest, found := strings.Cut(frame, workspaceOwnerSetupPrefix)
	if !found || len(rest) < 32 {
		t.Fatal("owner frame missing per-command setup marker")
	}
	for _, ch := range rest[:32] {
		if !strings.ContainsRune("0123456789abcdef", ch) {
			t.Fatal("owner frame has malformed setup marker")
		}
	}
	return workspaceOwnerSetupPrefix + rest[:32]
}
...
func TestWorkspaceOwnerWSL2BackgroundCommandsStreamExpandedScripts(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "background workload", remote: strings.Repeat("printf 'background'\n", 512)},
		{name: "rsync guard", remote: owner.rsyncGuardPayload("/workspace/project")},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			dir := installWorkspaceOwnerRecordingSSH(t)

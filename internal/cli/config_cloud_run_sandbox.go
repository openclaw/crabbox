...
type CloudRunSandboxConfig struct {
	...
	CLIPath     string `config:"cliPath" env:"CRABBOX_CLOUD_RUN_SANDBOX_CLI" envAlias:"CLOUD_RUN_SANDBOX_BINARY" flag:"cloud-run-sandbox-cli" sources:"user,repo,env,flag" help:"path to the Cloud Run sandbox CLI binary (direct mode)" default:"/usr/local/gcp/bin/sandbox" fileIgnoreEmpty:"true"`
	Workdir     string `config:"workdir" env:"CRABBOX_CLOUD_RUN_SANDBOX_WORKDIR" flag:"cloud-run-sandbox-workdir" sources:"user,repo,env,flag" help:"absolute working directory inside the sandbox (sync target)" default:"/tmp/crabbox" fileIgnoreEmpty:"true"`
	...
	Rootfs      string `config:"rootfs" env:"CRABBOX_CLOUD_RUN_SANDBOX_ROOTFS" flag:"cloud-run-sandbox-rootfs" sources:"user,repo,env,flag" help:"root filesystem exposed to the sandbox (default /)" default:"/" fileIgnoreEmpty:"true"`
}

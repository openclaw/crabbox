...
func TestArtifactPublisherCommandsScrubTargetEnvironmentAndPreserveToolAuth(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		authCheck string
		publish   func(context.Context, artifactPublishOptions, string) error
	}{
		{
			name:    "aws",
			command: "aws",
			...
			publish: func(ctx context.Context, opts artifactPublishOptions, dir string) error {
				opts.Bucket = "qa"
				_, err := uploadArtifactS3(ctx, opts, filepath.Join(dir, "proof.png"), "proof.png")
				return err
			},
		},
		{
			...
			publish: func(ctx context.Context, opts artifactPublishOptions, dir string) error {
				...
				_, err := uploadArtifactCloudflare(ctx, opts, filepath.Join(dir, "proof.png"), "proof.png")
				return err
			},
		},
		{
			...
			publish: func(ctx context.Context, opts artifactPublishOptions, dir string) error {
				...
				if err := os.WriteFile(bodyPath, []byte("proof"), 0o600); err != nil {
					return err
				}
				return postGitHubPRComment(ctx, opts, bodyPath)
			},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := tc.publish(context.Background(), opts, dir); err != nil {
				t.Fatal(err)
			}
		})
	}

...
func TestProvidersDescribeBuiltBinaryContract(t *testing.T) {
	...
	const baselineSHA256 = "9765fa4663d88f95b3feae4d6f159966930bbb31c45c0e561399c975cbdd3eef"
	if got := hex.EncodeToString(digest[:]); got != baselineSHA256 || len(helpStderr) != 61849 {
		t.Fatalf("run --help changed: sha256=%s bytes=%d, want sha256=%s bytes=61849", got, len(helpStderr), baselineSHA256)
	}
}

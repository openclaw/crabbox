...
func TestWindowsGitCoherenceGeneration(t *testing.T) {
	...
	for _, want := range []string{
		...
		"git read-tree --reset",
		"git write-tree",
		"& git update-ref $oldSym $target $oldHead",
		"& git update-ref --no-deref HEAD $target $oldHead",
		"& git update-ref $oldSym $oldHead $target",
		"& git update-ref --no-deref HEAD $oldHead $target",
		"Git symbolic HEAD changed during coherence",
	...
	if strings.Contains(decoded, "& git symbolic-ref HEAD $oldSym") {
		t.Fatalf("Windows coherence detaches and reattaches a symbolic HEAD:\n%s", decoded)
	}
}
...
func TestWindowsGitCoherenceRollbackRestoresIndexAfterSymbolicHeadFailure(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "symbolic", symbolic: true, injectRollbackFailure: true, wantError: "Git HEAD rollback failed"},
...
function git {
  ...
  if ($script:headUpdateObserved -and -not $script:postHeadFailureInjected -and $isPostHeadVerify) {
    ...
    if ($script:expectedSymbolicHead) {
      ...
      if ($symExit -ne 0 -or ([string]$installedSym).Trim() -ne $script:expectedSymbolicHead -or $branchExit -ne 0 -or ([string]$installedBranch).Trim() -ne $script:expectedTarget) {
        throw "Git shim did not observe the symbolic HEAD branch at the target"
      }
  ...
  $isBranchRollback = $gitArgs.Count -ge 4 -and $verb -eq 'update-ref' -and $gitArgs[1] -eq $script:expectedSymbolicHead -and $gitArgs[2] -eq $script:expectedOldHead -and $gitArgs[3] -eq $script:expectedTarget
  if ($script:injectRollbackFailure -and $script:postHeadFailureInjected -and $isBranchRollback -and -not $script:branchRollbackFailureInjected) {
    ...
    if ($symExit -ne 0 -or ([string]$restoredSym).Trim() -ne $script:expectedSymbolicHead -or $branchExit -ne 0 -or ([string]$restoredBranch).Trim() -ne $script:expectedOldHead) {
      throw "Git shim did not restore the symbolic HEAD branch before fault injection"
    }
  ...
  $isSymbolicUpdate = $gitArgs.Count -ge 4 -and $verb -eq 'update-ref' -and $script:expectedSymbolicHead -and $gitArgs[1] -eq $script:expectedSymbolicHead -and $gitArgs[2] -eq $script:expectedTarget -and $gitArgs[3] -eq $script:expectedOldHead
  ...
  if ($gitExit -eq 0 -and ($isSymbolicUpdate -or $isDetachedUpdate)) {

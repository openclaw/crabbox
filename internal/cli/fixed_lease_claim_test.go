		...
		t.Run(name, func(t *testing.T) {
			...
			err := WithDurableLeaseClaimLock(id, func(c *LeaseClaim, _ bool, persist func() error) error {
				*c = LeaseClaim{
					...
					CoordinatorRegistrationURL: "https://coordinator.example.test", RuntimeAdapterRegistrationID: "registration-current",
					RuntimeAdapterPendingRegistrationID: "registration-next",
			...
			// Registered adapters complete coordinator deletion from the receipt.
			if terminal.CoordinatorRegistrationURL != claim.CoordinatorRegistrationURL ||
				terminal.RuntimeAdapterRegistrationID != "registration-current" || terminal.RuntimeAdapterPendingRegistrationID != "registration-next" {
				t.Fatalf("terminal claim lost coordinator registration: %#v", terminal)
			}
			rebound := terminal
			rebound.RuntimeAdapterRegistrationID = "registration-other"
			if err := kind.ValidateTerminalClaim(rebound, claim, id, nil); err == nil {
				t.Fatal("changed registration generation accepted")
			}
			if retain {

...
func TestSparseCheckoutIncludedPathsUsesSanitizedGitEnvironment(t *testing.T) {
	dir := t.TempDir()
	runGit(t, dir, "init")
	runGit(t, dir, "config", "user.email", "test@example.com")
	runGit(t, dir, "config", "user.name", "Test")
	...
	// An inherited malformed GIT_CONFIG_COUNT makes git fail before reading
	// repository rules. Sparse evaluation must use repositoryGitEnvironment.
	t.Setenv("GIT_CONFIG_COUNT", "1")
	included, err := sparseCheckoutIncludedPaths(dir, []gitTrackedPath{
		{name: "visible/keep.txt"},
		{name: "hidden/drop.txt"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := included["visible/keep.txt"]; !ok {
		t.Fatalf("included paths=%v, missing visible path", included)
	}
	if _, ok := included["hidden/drop.txt"]; ok {
		t.Fatalf("included paths=%v, should omit hidden path", included)
	}
}
...
func TestSyncManifestIgnoresGitlinksButWholeCheckoutGuardDoesNot(t *testing.T) {
	...
	if len(manifest.Deleted) != 0 {
		t.Fatalf("manifest deleted=%v", manifest.Deleted)
	}
	if omitted, err := GitCheckoutHasHiddenOmissions(dir); err != nil || !omitted {

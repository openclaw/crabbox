...
type StaticConfig struct {
	...
	Power                StaticPowerConfig `sources:"runtime"`
	PowerAcknowledgeStop bool              `sources:"runtime"`
}

type StaticPowerConfig struct {
...
type fileStaticSection struct {
	...
	Power            *StaticPowerConfig `yaml:"power,omitempty"`
}
...
func applyStaticFileConfig(cfg *Config, file *fileStaticSection, source configInputSource, credentialSource credentialValueSource) error {
	...
	if file.Power != nil {
		cfg.Static.Power = *file.Power
		if credentialSource == credentialSourceTrustedFile {
			provenance.staticPowerApproval = *file.Power
		}
		provenance.staticPower = credentialSource
	}
	if file.User != "" {
		provenance.staticUser = credentialSource
	}
	if file.Port != "" {
		provenance.staticPort = credentialSource
	}
...
func applyStaticEnvironmentConfig(cfg *Config) error {
	...
	if os.Getenv("CRABBOX_STATIC_USER") != "" {
		cfg.credentialProvenance.staticUser = credentialSourceEnvironment
	}
	if os.Getenv("CRABBOX_STATIC_PORT") != "" {
		cfg.credentialProvenance.staticPort = credentialSourceEnvironment
	}
	if err != nil {
		return err
	}
...
func validateStaticCommandSources(cfg Config) error {
	...
	if len(cfg.Static.StartCommand) > 0 || len(cfg.Static.StopCommand) > 0 {
		if !cfg.Static.Power.Dedicated {
			return Exit(2, "static power hooks require static.power.dedicated: true in trusted user config; this controller must exclusively own the host")
		}
		if provenance.staticPower == credentialSourceRepository && cfg.Static.Power != provenance.staticPowerApproval {
			return Exit(2, "repository-configured static.power requires the identical dedicated-host contract in trusted user config")
		}
		if cfg.Static.User == "" || cfg.Static.Port == "" {
			return Exit(2, "static power hooks require explicit static.user and static.port in trusted configuration")
		}
		if provenance.staticUser == credentialSourceRepository || provenance.staticPort == credentialSourceRepository || provenance.staticHost == credentialSourceRepository {
			return Exit(2, "static power hooks require host, user and port in trusted user config or explicit environment/flags; repository target overrides cannot inherit power authority")
		}
	}

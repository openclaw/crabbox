...
if ($LASTEXITCODE -ne 0 -or ([string]$actualOrigin).Trim() -ne $expectedOrigin) { exit 0 }
...
if ($LASTEXITCODE -ne 0) { & git update-ref -d $tmpRef 2>$null; $null = $LASTEXITCODE; exit 0 }
& git merge-base --is-ancestor $target $tmpRef 2>$null
$isAncestor = $LASTEXITCODE -eq 0
if (-not $isAncestor) { & git update-ref -d $tmpRef 2>$null; $null = $LASTEXITCODE; exit 0 }
$targetTree = & git rev-parse --verify ($target + "^{tree}") 2>$null
if ($LASTEXITCODE -ne 0 -or ([string]$targetTree).Trim() -ne $tree) { & git update-ref -d $tmpRef 2>$null; $null = $LASTEXITCODE; exit 0 }
...
try {
  ...
  if ($oldSym) {
    ...
    if ($LASTEXITCODE -ne 0 -or ([string]$installedSym).Trim() -ne $oldSym) { throw "Git symbolic HEAD changed during coherence" }
  }
} catch {
  $failure = $_
  $rollbackError = $null
  ...
  if ($headChanged) {
    if ($oldSym) {
      & git update-ref $oldSym $oldHead $target 2>$null
    } else {

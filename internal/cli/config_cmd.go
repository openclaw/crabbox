...
func configShowView(cfg Config) map[string]any {
	...
	return map[string]any{
		...
		"parallels": map[string]any{
			...
			"auth":             tokenState(cfg.Parallels.Password),
...
func writeConfigShowText(w io.Writer, cfg Config) {
	...
	fmt.Fprintf(w, "parallels template=%s source=%s source_id=%s snapshot=%s snapshot_id=%s clone_mode=%s host=%s user=%s work_root=%s startup_timeout=%s templates=%d hosts=%d auth=%s\n", blank(cfg.Parallels.Template, "-"), blank(cfg.Parallels.Source, "-"), blank(cfg.Parallels.SourceID, "-"), blank(cfg.Parallels.SourceSnapshot, "-"), blank(cfg.Parallels.SourceSnapshotID, "-"), cfg.Parallels.CloneMode, blank(cfg.Parallels.Host, "local"), cfg.Parallels.User, cfg.Parallels.WorkRoot, cfg.Parallels.StartupTimeout, len(cfg.Parallels.Templates), len(cfg.Parallels.Hosts), tokenState(cfg.Parallels.Password))
}
